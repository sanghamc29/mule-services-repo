...
Apart from CloudHub 2, logs are also forwarded to DataDog - as part of the default configuration in log4j2.xml. 

The following configuration must be applied when deploying an app for log forwrading to work:

* Property `log.aggregator` must be provided and set to `DataDog`
* Property `mule.env` must be provided and set to the deployment environment (DEV, STAGE, PROD) - this allows filtering mule logs based on environment in DataDog
* Secure property `datadog.api.key` must be provided and set with appropriate key - there is a different key for nonprod and prod environments

All the properties about are injected by the CD Pipeline by default - there is not additional work expected to be done by developers.

Running mule application localy does NOT forward any logs to DataDog.

Note: In DataDog use `source:mulesoft` to filter logs coming from MuleSoft apps.
